...
class ScenMode extends Component<Props, State> {
	...
	render() {
		...
		return (
			<div className="Scene">
				<div className="clear" />
				<Button className="Scene-reutrnBtn" type="primary" onClick={this._returnPage}>返回上一级</Button>
							...
							<div className="Scene-body-left-scen-header">
								...
								<div className="Scene-body-left-scen-header-img">
									<a
										...
										href={null}>
										<img
											src={require('./../../assets/img/jias.png')}
										/>
									</a>
									<a
									...
									<a disabled={!this.state._info.includes('delete')}
										onClick={this.info.bind(this, 'delete')} href={null}>
										<img
							...
							<div className="Scene-body-left-list-header">
								...
								<div className="Scene-body-left-list-header-left">
									<img
										...
										src={require('./../../assets/img/delete.png')}
									/>
									<Button disabled={!this.state._info.includes('save')} className="Scene-body-left-list-header-left-btn" onClick={this.save} type="primary">保存</Button>
								</div>
							</div>
							<DragDropContext
													...
													>
														{(provided, snapshot) => (
															<div
																...
																className={this.state.mark === index ? 'aaaaaaaa' : 'Scene-body-list-centent-item'}
																ref={provided.innerRef}
															...
															>
																...
																<Input
																	...
																	className="input"
																	onChange={e => { this.DeleteArray(e, index); }}
					...
					<div className="models">
						...
						<div style={{ marginTop: 10, }}>
							...
							<Select
								...
								style={{ width: '12rem' }} onChange={(e) => { this.setState({ SerialNumberCode: e, }), console.log(e); }}>
	...
	del = () => {
		...
		if (this.state.Delt !== null && typeof (this.state.Delt) !== 'undefined') {
			// console.log(this.state.Delt);
			// console.log(this.state.Delt.order_no + '当前点击');
			this.state.items.forEach((e, index) => {
				if (e.order_no !== this.state.Delt.order_no) {
					return items.push(e);
				}
			});
			let res = items.length === this.state.mark ? items.length - 1 : this.state.mark;
			// console.log(items);
			this.setState({
				items: _formatData(items), //这里从新遍历一次items数据，因为列表是更具items.order_no排序的，不然列表会乱窜！！
				Delt: items[res],
				mark: items.length === this.state.mark ? items.length - 1 : this.state.mark
			});
	...
	onDragEnd = (result: object) => {
		...
		console.log(items);
		this.setState({ items: _formatData(items), mark: result.destination.index, Delt: items[result.destination.index] });
	};
	//
		...
		if (this.state.seven !== null) {
			...
			this.state.items.forEach((element, index) => {
				antd.push({
					...
					key_value: typeof (element.key_value) !== 'undefined' ? element.key_value : ''
				});
			});
	...
	onOk = async () => {
		if (this.state.infoText !== null && this.state.SerialNumberCode !== null) {
			...
			let res = await Api.Scenario.save_scene(this.state.SerialNumberCode, this.state.infoText, window.localStorage.getItem('classroomid'), id); //this.props.location.query
			// console.log(res);
			if (res.code === 200) {
				window._guider.Utils.alert({
					message: res.msg,
