...
class DeviceViode extends Component<Props, State> {
	...
	_infos = () => {
		window.YjFPlayer = {
			onEvent: (str) => {
				console.log(str);
				if (str == 'PLAYER_INIT_DONE') { //默认情况下播放
					setTimeout(() => { this.props.DeviceDetection.refreshPlay(); }, 1);
				} else if (str == 'PLAYER_SCREEN_FULL') { //全屏情况下播放
					setTimeout(() => { this.props.DeviceDetection.FullScreen(); }, 1);
				} else if (str == 'PLAYER_SCREEN_NORMAL') { //退出全屏情况下播放
					setTimeout(() => { this.props.DeviceDetection.refreshPlay(); }, 1);
				}
			}
		};
	...
	render() {
		...
		const { InteractiveTablet, projector, curtainControl, pc, OPS } = this.props;
		return (
			<div className="devCtl-row-div">
				...
				<div>
					<div className="devCtl-row-div-vide">
						...
						<object classID="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="550" height="240" id="XKFlashPlayer">
							...
							{/* <!--[if !IE]>--> */}
							<object type="application/x-shockwave-flash" data="HWFlashPlayer.swf?param=2.0" width="550" height="240"
								id="XKFlashPlayer">
								<param name="quality" value="high" />
								<param name="bgcolor" value="#000000" />
								<param name="allowScriptAccess" value="always" />
            		...
            		<!--[if gte IE 6]>--> */}
								...
								<a href="http://www.adobe.com/go/getflashplayer">
									<img src={require("../../../assets/img/get_flash_player.gif")}
				...
				{/* 按钮 */}
				<div className="devCtl-row-div-pc" style={displayBtn === true ? { width: '47rem' } : { width: '25rem' }}>
					<div className="devCtl-row-div-pc-div">
						<span className="devCtl-row-div-pc-psan">
							{OPS.length > 0 ? OPS[0].equip_type_name : '电脑'}
						</span>
						<a className="devCtl-row-div-pc-div-img" href={null} onClick={this.folding}>
							{!this.state.displayBtn ? <img src={require('../../../assets/img/right.png')} />
								: <img src={require('../../../assets/img/left.png')} />}</a>
					...
					<div className="devCtl-row-div-pc-divs">
						...
						{/* 交互平板 */}
						<span className="devCtl-row-div-pc-psan">{InteractiveTablet.length > 0 && InteractiveTablet[0].equip_type_name}</span>
						<div className="devMap">
							{InteractiveTablet.map(this.pcView)}
						</div>
					</div>
				</div>
			</div>
