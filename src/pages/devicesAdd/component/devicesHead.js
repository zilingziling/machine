...
import { Button, Form, Table, Input, Select, TreeSelect, Modal, Radio } from 'antd';
...
export default class DeviceHeade extends Component<Props, State> {
		...
		return (
			<div className="deviceadd-body-headers">
				<div className="deviceadd-body-headers-seve">
					...
					<Button type="primary"
							onClick={this._Links}
							className="deviceadd-body-headers-seve-btn" style={deviceBtn}>设备控制</Button>
				</div>
				<div className="deviceadd-body-headers-add">
					<Button disabled={!this.state._info.includes('add_device')} onClick={this.ofs} type="primary" className="deviceadd-body-headers-add-btn">添加设备</Button>
					{/* <span className='deviceadd-body-headers-add-span'>批量导入</span> */}
					<Button type="primary"
							onClick={this.scenario}
							className="deviceadd-body-headers-seve-btn" style={sceneBtn}>情景编辑</Button>
					<Button disabled={!this.state._info.includes('Copy_data')} onClick={this._cydata} type="primary" className="deviceadd-body-headers-add-btns">拷贝数据</Button>
					...
					{mark === 1 ?
						<div>
							<AddInfo
								...
								ref="inofs" itmeid={(v, data) => { this.setState({ classinfo: v, Submit: data }); }} />
							<div style={{ textAlign: 'center' }}>
								<Button type="primary" className="btns" onClick={this.handleOk}>保存设备</Button>
							</div>
						</div>
						: <div className="select_cy">
							<div className="select_cy-row">
								...
								<TreeSelect
									onChange={(e) => { this.setState({ cpData: e }); }}
							...
							<div className="select_cy-toRow">
								<Button type="primary" className="btns" onClick={this.cpDataSeve}>保存</Button>
							</div>
						</div>}
				</BaseModel>
			</div>
	...
	cpDataSeve = async () => {
		if (this.state.cpData !== null || this.props.classroomId !== null) {
			// 
			try {
				let res = await Api.Device.CyData(this.props.classroomId, this.state.cpData);
				console.log(res);
				if (res.code === 200) {
					window._guider.Utils.alert({
						message: res.msg,
	...
	ofs = () => {
		if (typeof (this.props.classroomId) !== 'undefined') {
			this.setState({ visible: true, mark: 1 });
		} else {
			window._guider.Utils.alert({
				message: '请选择教室在添加设备',
				type: 'error'
...
class extends Component<State> {
	...
	render() {
		...
		return (
			<Form className="row" onSubmit={this.handleSubmit}>
				...
				<FormItem className="row" style={{ display: 'flex' }} {...formitems} label="设备类型">
					{getFieldDecorator('equiptypeid', {
						initialValue: itmeDAta.equiptypename
					})(
						<Input disabled className="add-inputs"  style={inputWidth}/>
					)}
					<Button type="primary" className="row-btn" onClick={() => { this.setState({ visible: true }); }}>
						选择设备
					</Button>
				</FormItem>
				...
				<FormItem className="row"{...formItemstyles} label="设备名称">
					{getFieldDecorator('equiptypename', {
						initialValue: typeof (itmeDAta.brandcnname) !== 'undefined' ? typeof (itmeDAta.equiptypename) !== 'undefined' ? itmeDAta.brandcnname + itmeDAta.equiptypename : null : null
					})(
				...
				{itmeDAta.equiptype === '1' ?
					<div>
						...
						<FormItem className="tiemsAdd" {...formItemstyles} label="服务器IP">
					...
					<div>
						<FormItem className="row"{...formItemstyles} label="地址格式">
							{getFieldDecorator('addrFormat', {
								initialValue: this.state.Address === null ? '通用' : this.state.Address
							})(
								<Select style={inputWidth} className="row-select" onChange={(e) => { this.setState({ Address: e }); }}>
						...
						{this.state.Address === 'RTSP' ?
							<FormItem className="row"{...formItemstyles} label="RTSP地址">
								{getFieldDecorator('rtspAddr', {
									...
									<TextArea style={inputWidth} maxLength={250} rows={4} className="add-inputs" />
								)}
							</FormItem>
							: null}
					</div>
					...
					<div>
						<FormItem className="row"{...formItemstyles} label="IP">
							{getFieldDecorator('hostIp', {
								...
								<Input style={inputWidth} maxLength={15} className="add-inputs" />
							)}
						</FormItem>
						<FormItem className="row"{...formItemstyles} label="端口号">
							{getFieldDecorator('portNo', {
								...
								<Input style={inputWidth} maxLength={5} className="add-inputs" />
							)}
						</FormItem>
					</div>
					: null
					...
					<div>
						<FormItem className="row"{...formItemstyles} label="用户名">
							{getFieldDecorator('username', {
								...
								<Input maxLength={20} className="add-inputs" style={inputWidth}/>
							)}
						</FormItem>
						<FormItem className="row"{...formItemstyles} label="密码">
							{getFieldDecorator('password', {
								...
								<Input style={inputWidth} maxLength={20} type="password" className="add-inputs" />
							)}
						</FormItem>
					</div>
					: null
		...
		return (
			<div>
				...
				<Table
					...
					rowClassName={(record) => {
						return this.state.itmeColor === record.key ? 'device-tables-tableRows' : null;//device-tables-tableRow';
					}}
					...
					onRow={(record, index) => {
						return {
							onClick: () => { this.setState({ itmeColor: record.key, item: record }); },       // 点击行
						};
					}}
				/>
				<Pagination
				...
				<div className="textAlign">
					<Button onClick={this.foramData} type="primary" className="btns">选择完成</Button>
				</div>
	...
	foramData = async () => {
		...
		if (this.state.itmeColor !== null) {
			...
			if (this.state.item.connecttype === '1' || this.state.item.connecttype === '2') {
				try {
					// let res = await Api.Device.editor(this.state.itmeColor);
					// let data = await Api.Device.dataCtr(this.state.item.equiptype, this.state.item.key);
					// let mark = indexName.indexOf(res.data.equiptypename) == -1 ? 'RTSP' : '通用';
	...
	list = async (pages: number, deviceValue: string, brandType: string, inpuValue: string, brandTypeId: string) => {
		let res = await Api.Device.DeviceList(pages, deviceValue, brandType, inpuValue, brandTypeId);
		if (res.code === 200) {
			// console.log(res);
